"""
Ce script encode toutes les frames extraites du dataset UCF-Crime avec le modèle CLIP d'OpenAI.

On sauvegarde trois fichiers à la fin :
    clip_embeddings.npy     les vecteurs de toutes les frames
    clip_metadata.csv       les infos associées à chaque frame (classe, vidéo, timestamp)
    clip_metrics.json       les performances du modèle (temps, vitesse...)

Le script supporte la reprise après interruption grâce à un système de checkpoint.
Un checkpoint est sauvegardé toutes les 10 minutes dans results/clip/clip_checkpoint.npz.
...
OUTPUT_DIR = Path("results/clip")

# Variante du modèle CLIP choisie
MODEL_NAME = "ViT-B/32"
...
class FrameDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        try:
            ...
            # On applique le préprocesseur de CLIP qui redimensionne et normalise l'image selon ce qu'attend le modèle
...
def save_checkpoint(output_dir, all_embeddings, all_metadata, errors, nb_frames_processed):
    ...
    checkpoint_path = output_dir / 'clip_checkpoint.npz'
    tmp_path = output_dir / 'clip_checkpoint_tmp.npz'
    ...
    meta_tmp = output_dir / 'clip_checkpoint_meta.csv'
...
def load_checkpoint(output_dir):
    ...
    checkpoint_path = output_dir / 'clip_checkpoint.npz'
    meta_path = output_dir / 'clip_checkpoint_meta.csv'
...
def cleanup_checkpoint(output_dir):
    ...
    for name in ['clip_checkpoint.npz', 'clip_checkpoint_meta.csv', 'clip_checkpoint_tmp.npz']:
        p = output_dir / name
        if p.exists():
            p.unlink()
...
def encode_frames(frames_dir, output_dir, model_name, batch_size):
    ...
    # CLIP télécharge les poids tout seul au premier appel, ils sont mis en cache donc les prochains lancements seront instantanés
    ...
    if checkpoint is not None:
        ...
        if len(frame_paths) == 0:
            ...
            # On passe directement à la sauvegarde finale
            encoding_time = 0
            nb_encoded = len(embeddings_matrix)
            # Sauvegarde finale (on saute la boucle)
    ...
    with torch.no_grad():  # on désactive le calcul des gradients car on n'entraîne pas le modèle
        for images, paths, valid_flags in tqdm(loader, desc="CLIP"):
...
def _save_final_results(output_dir, embeddings_matrix, all_metadata, errors,
    ...
    embeddings_path = output_dir / 'clip_embeddings.npy'
