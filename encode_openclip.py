"""
Ce script encode toutes les frames extraites du dataset UCF-Crime avec le modèle Evaclip.

On sauvegarde trois fichiers à la fin dans results/<nom_sortie>/ :
    <nom_sortie>_embeddings.npy   les vecteurs de toutes les frames
    <nom_sortie>_metadata.csv     les infos associées à chaque frame (classe, vidéo, timestamp)
    <nom_sortie>_metrics.json     les performances du modèle (temps, vitesse...)
...
OUTPUT_DIR = Path(f"results/{NOM_SORTIE}")
...
class FrameDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        try:
            ...
            # On applique le préprocesseur du modèle qui redimensionne et normalise
            # l'image selon ce qu'attend le modèle open_clip
            return self.preprocess(image), str(path), True
        except Exception:
            # Si une image est corrompue ou illisible on retourne un tensor vide
            # plutôt que de faire planter tout le script
            dummy = torch.zeros(3, 224, 224)
            return dummy, str(path), False


def save_checkpoint(output_dir, nom_sortie, all_embeddings, all_metadata, errors, nb_frames_processed):
    """Sauvegarde un checkpoint pour pouvoir reprendre plus tard."""
    checkpoint_path = output_dir / f'{nom_sortie}_checkpoint.npz'
    tmp_path        = output_dir / f'{nom_sortie}_checkpoint_tmp.npz'
    ...
    # On sauvegarde d'abord dans un fichier temporaire puis on renomme pour éviter la corruption
    ...
    meta_tmp = output_dir / f'{nom_sortie}_checkpoint_meta.csv'
...
def load_checkpoint(output_dir, nom_sortie):
    """Charge un checkpoint existant. Retourne None s'il n'y en a pas."""
    checkpoint_path = output_dir / f'{nom_sortie}_checkpoint.npz'
    meta_path       = output_dir / f'{nom_sortie}_checkpoint_meta.csv'
...
def cleanup_checkpoint(output_dir, nom_sortie):
    ...
    for suffix in ['_checkpoint.npz', '_checkpoint_meta.csv', '_checkpoint_tmp.npz']:
        p = output_dir / f'{nom_sortie}{suffix}'
        if p.exists():
            p.unlink()
...
def encode_frames(frames_dir, output_dir, model_name, model_pretrained, nom_sortie, batch_size):
    ...
    # open_clip télécharge automatiquement les poids au premier lancement
    # et les met en cache dans ~/.cache/huggingface/ ou ~/.cache/open_clip/
    logger.info(f"Chargement du modèle {model_name} ({model_pretrained})...")
    ...
    resume_checkpoint = load_checkpoint(output_dir, nom_sortie)
    skip_frames       = 0
    ...
    if resume_checkpoint is not None:
        ...
        if len(frame_paths) == 0:
            ...
            _save_final_results(output_dir, embeddings_matrix, all_metadata, errors,
                                ...
                                nom_sortie, batch_size, device)
            cleanup_checkpoint(output_dir, nom_sortie)
            return
    else:
        logger.info("Aucun checkpoint trouvé, démarrage depuis le début")

    # Le DataLoader va charger les images par batchs et en parallèle
    # ce qui est bien plus rapide que de les charger une par une
    ...
    # On démarre le chrono ici pour mesurer uniquement le temps d'encodage pur
    # sans compter le chargement du modèle
    ...
    with torch.no_grad():  # on désactive le calcul des gradients car on n'entraîne pas le modèle
        for images, paths, valid_flags in tqdm(loader, desc=f"{model_name}"):
            ...
            # open_clip utilise encode_image comme CLIP et MobileCLIP
            embeddings = model.encode_image(valid_images)

            # On normalise les vecteurs pour que leur norme soit égale à 1
            # c'est nécessaire pour que la similarité cosinus fonctionne correctement avec FAISS
            embeddings = embeddings / embeddings.norm(dim=-1, keepdim=True)

            all_embeddings.append(embeddings.cpu().float().numpy())

            # On extrait les métadonnées depuis le nom de fichier pour pouvoir retrouver
            # à quelle vidéo et à quel moment appartient chaque frame
            ...
            if now - last_checkpoint_time >= CHECKPOINT_INTERVAL:
                ...
                save_checkpoint(output_dir, nom_sortie, all_embeddings, all_metadata,
    ...
    # On concatène tous les batchs en une seule matrice numpy
...
def _save_final_results(output_dir, embeddings_matrix, all_metadata, errors,
    ...
    embeddings_path = output_dir / f'{nom_sortie}_embeddings.npy'
    ...
    # Sauvegarde des métadonnées, l'index de chaque ligne correspond
    # à la position du vecteur dans la matrice d'embeddings
    metadata_path = output_dir / f'{nom_sortie}_metadata.csv'
    pd.DataFrame(all_metadata).to_csv(metadata_path, index=True)
    logger.info(f"Métadonnées sauvegardées dans {metadata_path}")

    # Sauvegarde des métriques de performance, ce fichier sera comparé
    # avec ceux des autres modèles pour les graphiques
    metrics = {
    ...
    metrics_path = output_dir / f'{nom_sortie}_metrics.json'
